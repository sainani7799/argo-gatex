...
const DCForm = (props: DCFormProps) => {
  ...
  const getWarehouses = async () => {
    try {
      ...
      const activeWarehouses = res.data.filter(
  ...
  useEffect(() => {
    ...
    form.getFieldValue('addresserNameId'),
    form.getFieldValue('toDepartmentId'),
  ]);
  ...
  const getAllToAddressByUnit = async (radioValue) => {
    ...
    addressService
      ...
      .then((res) => {
        console.log(res);
        if (res) {
          const activeToAddress = res.data.filter(
      ...
      .then((res) => {
        if (res) {
          const activeSuppliers = res.data.filter(
            (suppliers) =>
              suppliers.isActive === true && suppliers.type === 'SUPPLIER'
          );
          setSuppliers(activeSuppliers);
          const activeBuyers = res.data.filter(
            (buyer) => buyer.isActive === true && buyer.type === 'BUYER'
          );
  ...
  const handleDCTypeChange = (value) => {
    setIsReturnable(value === 'returnable'); // Set state to true if returnable is selected
  };
    ...
    form
      ...
      .then(() => {
        ...
        if (parseFloat(value) <= 50000) {
          const req = new DcReq(
            form.getFieldValue('fromUnitId'),
            form.getFieldValue('warehouseId'),
            form.getFieldValue('departmentId'),
            form.getFieldValue('poNo'),
            form.getFieldValue('modeOfTransport'),
            form.getFieldValue('toAddresser'),
            form.getFieldValue('addresserNameId'),
            form.getFieldValue('weight'),
            form.getFieldValue('vehicleNo'),
            form.getFieldValue('returnable'),
            form.getFieldValue('purpose'),
            form.getFieldValue('value'),
            StatusEnum.ASSIGN_TO_APPROVAL,
            form.getFieldValue('requestedBy'),
            form.getFieldValue('remarks'),
            form.getFieldValue('createdUser'),
            itemTableData,
            '',
            AcceptableEnum.NO,
            null,
            form.getFieldValue('attentionPerson'),
            form.getFieldValue('toDepartmentId'),
            form.getFieldValue('buyerTeam'),
            form.getFieldValue('dcType'),
            form.getFieldValue('expectedReturnDate')
          );
          ...
          message.error(
            `DC is can't be created because DC value more then 50,000`
          );
        }
      })
      .catch(() => {
  ...
  return (
    <>
      ...
      >
          ...
          >
            <Col xs={24} sm={12} md={8} lg={8} xl={8}>
              ...
              <Form.Item
                name="dcType"
                label="DC Type"
                rules={[{ required: true, message: 'Please Select DC Type' }]}
              >
                <Select
                  showSearch
        ...
        >
          ...
          />
              ...
              >
                ...
                >
                  {addressData?.map((unit) => {
                    return (
                      <Option
                        key={unit.addresser_name_id}
                        value={unit.addresser_name_id}
                      >
                        {unit.addresserName}
                      </Option>
                    );
            ...
            <Col xs={24} sm={12} md={8} lg={8} xl={8}>
              <Form.Item
                name="weight"
                label="Weight (KGS-Approximately)"
                rules={[
                  { required: true },
                  {
        ...
        <br />
        <Card>
          <Form layout="vertical" onFinish={onItemAdd} form={itemForm}>
            {' '}
            <h1
              style={{ color: '#6b54bf', fontSize: '15px', textAlign: 'left' }}
            >
              ITEM DETAILS
            </h1>
            <Row gutter={14}>
            ...
            <Row justify={'end'}>
              <Button disabled={addVisible} type="primary" htmlType="submit">
                {btnType}
              </Button>
            </Row>
          </Form>
